...
namespace wpf_dotnet
{
    public partial class InteractRoleControl : UserControl
    {
            ...
            try
            {
                ...
                foreach (var user in data.users)
                {
                    var roomUser = new RoomUser
                    {
                        ...
                        OriginalRoleId = user.role == "administrator" ? 1 : user.role == "moderator" ? 2 : 3 // Sauvegarde du r�le original
                    };
            ...
            try
            {
                ...
                foreach (RoomUser user in _moderatorUsers.Where(u => u.OriginalRoleId != 2))
                {
                    ...
                    else
                    {
                        ...
                        errorMessage.AppendLine($"� �chec promotion de {user.Username}: {errorContent}");
                    }
                }
                ...
                foreach (RoomUser user in _basicUsers.Where(u => u.OriginalRoleId != 3))
                {
                    ...
                    else
                    {
                        hasErrors = true;
                        var errorContent = await response.Content.ReadAsStringAsync();
                        errorMessage.AppendLine($"� �chec r�trogradation de {user.Username}: {errorContent}");
                    }
                }
                ...
                if (hasErrors)
                {
                    ...
                    fullMessage.AppendLine("Vous n'avez pas les permissions n�cessaires pour certains changements de r�le.");
                ...
                if (successCount > 0)
                {
                    MessageBox.Show($"{successCount} changement(s) de r�le appliqu�(s) avec succ�s !",
                                    "Succ�s",
                                    MessageBoxButton.OK,
        ...
        public class RoomUser
        {
            ...
            public int OriginalRoleId { get; set; } // Nouvelle propri�t�
